...
**⛔ HARD CLIENT RULE (2026-07-15): ALL GENERAL tools/apparatus AND all raw reagents are ALWAYS present on the bench across EVERY experiment — NEVER hide, remove, declutter, or per-experiment-gate the general apparatus/reagents.** A real lab keeps every instrument out at all times; the player is meant to CHOOSE the right ones. Reduce confusion with labels/hints/highlights, **never** by removing items. TWO narrow exceptions ONLY: (1) the 9 finished **END PRODUCTS** are demo-only (`EndProductVisibility`); (2) the **4 methane-only staged props** — `Prop_reagent-jar` (acetate+soda lime), `Prop_glass-tube`, `Prop_collection-tube`, `Prop_burner` — appear only in the **editor, Lab Tour and the Methane tutorial** (`MethaneStageVisibility`; nothing else uses them). The mortar/pestle/scoop and every kit item stay out permanently.

**Key scene objects (SampleScene):** ExperimentSystems (runner/launcher/builder/monitor/recorder) · HudRig · RobotNPC (Pharmee + gatekeeper) at the corridor corner · LabDoorController + door triggers + FrontDoorSpawn · PPELocker · ScreenFader · Services · PostLabTablet + GradeScreen (review corner) · MethaneStage + DynamicStage · ReagentShelf (west 3x4 cubby) + ReagentCabinets (east): raw materials always present, the 9 end products **demo-only** (client rule — synthesise from raw; `EndProductVisibility`) · WorkspaceShelf (gantry platforms) · WorldLabels · XR Origin (CC r0.25 + HeadCollisionPushback). MainMenu: cube room + MenuCanvas. Build Settings = [MainMenu(0), SampleScene(1)].
