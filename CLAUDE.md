...
- **Logs are LIVING, not append-only**: when a feature changes, UPDATE the affected lines in `systems-reference.md` / `gameplay-flow.md` / `experiments-reference.md` in place — never append dated narrative blobs. `Docs/changelog.md` gets **one line per batch** (date · name · 1-sentence summary · suite count). The WHY of a decision lives in code comments at the decision site + suite assertions, not in prose logs. Checklist items: flip `[x]` + ≤1-line note; fold long-completed sections into a single DONE line.
