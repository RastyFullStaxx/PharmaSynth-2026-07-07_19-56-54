...
- ⏳ NEXT (W2 batch 2): TaskGraph engine (phases A–D, prereqs, world-state auto-check) + BKT MasteryModel + ScoreCalculator; extend ExperimentModuleDefinition; wrong-reagent context-aware validation; extend ChemicalData/ReactionRule data model; apply liquid material to ChemLab `_WithLiquid` prefabs; user to manually verify play mode in headset when available.
