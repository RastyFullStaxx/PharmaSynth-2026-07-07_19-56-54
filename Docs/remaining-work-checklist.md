...
- [x] **Action & apparatus SFX set** (user request 2026-07-09) — CLIPS + ONE-SHOT HOOKUPS DONE: all 13 clips wired (SoundBank 31 entries) AND triggered in gameplay: footsteps (`FootstepPlayer` on the XR rig, 0.75 m stride, teleport-guarded), grab (`GrabPhysicsPolicy.selectEntered`), material-aware drop clatter (`ImpactSound`: glass clink / metal / wood, silent above break speed where shatter takes over), door-swing (`DoorOpener.SetOpen`), ppe-rustle (`PPEController.DonPPE`), socket-snap (`SelectSfx` on station sockets), reaction cues (`LiquidPhysics`: fizz for gas outcomes, mixture-complete chime for colour/ppt/odour, silence for negative tests — `Mishandling.SfxForOutcome`). Ambient hum auto-plays via the existing `AmbientPlayer`. Fix-pass extended: all 51 shelf/bench items now carry DropRespawn + BreakableGlassware (incl. `Reagent_*` bottles) + SpillMistake (pourables) + ImpactSound. *Remaining (small): sustained sim LOOPS (boil under `TemperatureSim`, filter drip, gas hiss, crystallise shimmer during their sim verbs) need loop lifecycle on `ZoneSimStation` — one-shots exist; stir hookup awaits a stirring verb.*
