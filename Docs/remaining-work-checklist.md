...
- [ ] **Action & apparatus SFX set** (user request 2026-07-09) — **ALL 13 CLIPS GENERATED & WIRED** (SoundBank now 31 entries): stir, footstep, grab, drop-metal, drop-wood, reaction-fizz, mixture-complete, crystallise, filter-drip, gas-hiss, ppe-rustle, door-swing, socket-snap (+ glass-clink/shatter pre-existing). **Remaining: the trigger hookups** — footsteps ↔ locomotion, grab/drop clatter ↔ `GrabPhysicsPolicy` + collision impulse (material-aware), stir ↔ vessel swirl, boil loop ↔ `TemperatureSim` at target, reaction-fizz ↔ `ReactionRule` fired, crystallise/filter/gas ↔ their sim rigs, ppe-rustle ↔ `PPEController`, door-swing ↔ `DoorOpener`, socket-snap ↔ socket select, mixture-complete ↔ `LiquidTaskBinding` full delivery. Clips need a human listen in Play mode.
